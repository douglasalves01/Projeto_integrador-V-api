...
class RecommendationService:
    ...
    async def get_recommendations(
        ...
        if jwt:
            try:
                ...
                if ai is not None and ai.available:
                    payload = await ai.get_recommendations(user_id, jwt=jwt, k=10)
        ...
        # 2) Fallback: algoritmo classico ja existente
    ...
    async def _materialize_ai_recommendations(
        ...
        """Converte payload da IA em entidades Recommendation persistidas."""
        items = payload.get("recommendations", [])
        if not items:
            return []

        valid_video_ids: set[UUID] = set()
        for item in items:
            try:
                valid_video_ids.add(UUID(str(item["content_id"])))
        ...
        existing = await db.execute(
            select(Video.id).where(Video.id.in_(valid_video_ids))
        )
